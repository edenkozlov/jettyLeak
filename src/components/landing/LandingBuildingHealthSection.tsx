...
const PILLAR_ROWS = [
  { key: 'stability' as const, v: 82 },
  { key: 'hydraulic' as const, v: 76 },
  { key: 'appliance' as const, v: 74 },
  { key: 'mechanical' as const, v: 88 },
]
...
export function LandingBuildingHealthSection({
  ...
  const exampleName = t('buildingHealth.example.name')
  const exampleCity = t('buildingHealth.example.city')
      ...
      <div className="relative mx-auto max-w-5xl px-4 sm:px-6">
        ...
        <div className="mx-auto mt-8 max-w-lg lg:mx-0 lg:mt-10 lg:max-w-none lg:grid lg:grid-cols-2 lg:items-stretch lg:gap-5">
          <div
            ...
            aria-label={t('buildingHealth.exampleAria', { name: exampleName })}
          >
            <div className="flex items-start justify-between gap-3 border-b border-gray-100 pb-2.5">
              <div className="min-w-0">
                <p className="text-[13px] font-semibold leading-snug text-gray-900">{exampleName}</p>
                <p className="mt-0.5 text-[12px] text-gray-500">{exampleCity}</p>
              </div>
              <p className="shrink-0 pt-0.5 text-[10px] font-medium tabular-nums text-gray-400">
                {t('buildingHealth.today')}
              </p>
            </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-1.5 sm:gap-2">
              {PILLAR_ROWS.map((row) => (
                ...
                >
                  <p className="text-[9px] font-medium tracking-wide text-gray-500 uppercase">
                    {t(`buildingHealth.pillars.${row.key}`)}
                  </p>
          ...
          <div className="mt-6 flex min-h-0 flex-col gap-3 lg:mt-0 lg:h-full">
            <LandingBhiTrendChart
              fillHeight
              buildingName={exampleName}
              buildingLocation={exampleCity}
            />
            <button
              type="button"
              ...
              aria-label={t('buildingHealth.scoreHowBuiltAria')}
            >
              <span>{t('buildingHealth.scoreHowBuilt')}</span>
