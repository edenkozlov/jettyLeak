...
const AUDIENCE_KEYS = ['propertyManagers', 'commercial', 'multifamily', 'industrial'] as const

const AUDIENCE_ICONS: Record<(typeof AUDIENCE_KEYS)[number], ReactNode> = {
  propertyManagers: (
    <>
    ...
    <div className="mt-12 grid gap-3 sm:mt-16 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
      {AUDIENCE_KEYS.map((key) => (
        ...
        >
          ...
          <h3 className="text-[15px] font-semibold">{t(`howItWorks.cards.${key}.label`)}</h3>
          <p className="mt-2 text-[13px] leading-relaxed text-white/35">
            {t(`howItWorks.cards.${key}.detail`)}
          </p>
        </div>
      ))}
    </div>
    ...
    >
      ...
      <div className="relative mx-auto max-w-5xl px-4 sm:px-6">
        <div className="mx-auto max-w-2xl text-center">
          <p className="text-xs font-semibold tracking-[0.3em] text-indigo-400 uppercase">
