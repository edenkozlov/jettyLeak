-- Track latest flow rate on nightwatch state (for sensor-state when window is empty).

ALTER TABLE public.jetty_nightwatch_state
  ADD COLUMN IF NOT EXISTS last_lpm double precision NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS last_flow_at timestamptz;
...
BEGIN
  ...
  IF p_flow_samples IS NOT NULL AND jsonb_array_length(p_flow_samples) > 0 THEN
    SELECT COALESCE(MAX((s->>'lpm')::double precision), 0) INTO v_max_lpm
  ...
  IF p_sim_mode IS NOT NULL OR p_force_night IS NOT NULL OR v_flow_count > 0 THEN
    INSERT INTO public.jetty_nightwatch_state (
      sensor_id, sim_mode, force_night, last_lpm, last_flow_at, updated_at
    )
    VALUES (
      p_sensor_id,
      COALESCE(p_sim_mode, 'idle'),
      COALESCE(p_force_night, false),
      v_max_lpm,
      CASE WHEN v_flow_count > 0 THEN now() ELSE NULL END,
      now()
    )
    ON CONFLICT (sensor_id) DO UPDATE SET
      ...
      END,
      last_lpm = CASE
        WHEN v_flow_count > 0 THEN v_max_lpm
        ELSE jetty_nightwatch_state.last_lpm
      END,
      last_flow_at = CASE
        WHEN v_flow_count > 0 THEN now()
        ELSE jetty_nightwatch_state.last_flow_at
      END,
      updated_at = now();
  END IF;
